...
import { STATUS_META, PRIORITY_META, formatDate, formatDateTime, fullName, monthLabel, currentMonthKey } from '@/lib/utils';
      ...
      <div className="fixed top-0 right-0 bottom-0 w-[560px] max-w-[94vw] bg-surface shadow-2xl flex flex-col" onClick={(e) => e.stopPropagation()}>
        <div className="p-3.5 border-b border-border flex items-center gap-2">
          ...
          <select value={task.status} onChange={(e) => updateField({ status: e.target.value as Task['status'] })} className="text-xs border border-border rounded-md px-2 py-1 bg-surface2">
            {Object.entries(STATUS_META).filter(([id]) => id === 'review' || id === 'done').map(([id, m]) => <option key={id} value={id}>{m.label}</option>)}
          </select>
        ...
        <div className="flex-1 overflow-y-auto p-4">
          ...
          <div className="grid grid-cols-2 gap-3 my-4">
            ...
            <Field label={task.status === 'review' ? 'Месяц' : 'Дедлайн'}>
              {task.status === 'review' ? (
                <MonthSelect
                  value={task.due_month ?? currentMonthKey()}
                  onChange={(due_month) => updateField({ due_month, due_date: null })}
                ...
                <input
                  ...
                  onChange={(e) => updateField({ due_date: e.target.value ? new Date(e.target.value + 'T12:00:00').toISOString() : null, due_month: null })}
                  className="w-full border border-border bg-surface2 rounded-md px-2 py-1.5 text-[13px]"
                />
              )}
            </Field>
